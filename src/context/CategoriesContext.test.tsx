...
describe('CategoriesContext', () => {
  ...
  test('listens for lgpd:requiredCategories event and cleans up', () => {
    ...
    expect(addEventListenerSpy).toHaveBeenCalledWith('lgpd:requiredCategories', expect.any(Function));

    unmount();

    expect(removeEventListenerSpy).toHaveBeenCalledWith('lgpd:requiredCategories', expect.any(Function));
  });

  test('logs cookie discovery in development', () => {
    const discoverRuntimeCookiesSpy = jest.spyOn(cookieDiscovery, 'discoverRuntimeCookies').mockReturnValue([]);
    const detectConsentCookieNameSpy = jest.spyOn(cookieDiscovery, 'detectConsentCookieName').mockReturnValue('cookie-consent');
    ...
    expect(consoleGroupSpy).toHaveBeenCalledWith('[🍪 LGPD-CONSENT] 🔎 Descoberta de cookies (experimental)');
